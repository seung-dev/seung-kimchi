...
public class SAlgorithm {
	...
	public static List<SLinkedHashMap> algorithms(
		...
		return Arrays.stream(Security.getProviders())
				.filter(provider -> SText.is_empty(provider_name) || provider.getName().toLowerCase().contains(provider_name.toLowerCase()))
				.flatMap(provider -> {
					return provider.getServices().stream()
						.filter(service -> SText.is_empty(include_name) || service.getAlgorithm().toLowerCase().contains(include_name.toLowerCase()))
						.filter(service -> SText.is_empty(exclude_name) || !service.getAlgorithm().toLowerCase().contains(exclude_name.toLowerCase()))
						.filter(service -> SText.is_empty(include_type) || service.getType().toLowerCase().contains(include_type.toLowerCase()))
						.map(service -> {
							return new SLinkedHashMap()
								.add("name", service.getAlgorithm())
								.add("type", service.getType())
